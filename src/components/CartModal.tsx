...
export const CartModal: React.FC<CartModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#2F3428]/70 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
      <div className="bg-[#F9F9F6] w-full max-w-lg h-full shadow-2xl flex flex-col justify-between overflow-hidden border-l border-[#A1A696]/30 relative">
        
        {/* Header */}
        <div className="bg-[#525A43] text-white p-5 flex items-center justify-between border-b border-[#A1A696]/30">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-lg bg-[#A1A696] text-[#2F3428] flex items-center justify-center font-bold">
              <ShoppingBag className="w-5 h-5 fill-[#2F3428]/20" />
            </div>
            <div>
              ...
              <p className="text-xs text-[#A1A696]">
          ...
          <button 
            ...
            className="p-2 rounded-lg text-[#A1A696] hover:text-white hover:bg-[#3F4633] transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <>
              {cartItems.length === 0 ? (
                <div className="text-center py-16 space-y-4">
                  <div className="w-20 h-20 mx-auto rounded-full bg-[#A1A696]/20 flex items-center justify-center text-[#525A43]">
                    <ShoppingBag className="w-10 h-10" />
                  </div>
                  <h3 className="text-lg font-bold text-[#2F3428]">Your cart is empty</h3>
                  <p className="text-xs text-stone-500 max-w-xs mx-auto">
                    Browse our authentic Tib-e-Nabvi and Unani herbal medicines to start shopping.
                  </p>
                  <button
                    onClick={onClose}
                    className="mt-4 px-6 py-2.5 rounded-xl bg-[#525A43] text-white font-bold text-xs hover:bg-[#3F4633] transition-colors"
                  >
                    Explore Products
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  {cartItems.map((item) => (
                    <div 
                      key={item.product.id}
                      className="bg-white p-3.5 rounded-2xl border border-stone-200/80 shadow-sm flex items-center gap-3"
                    >
                      <img 
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-[#2F3428] text-xs truncate">
                          {item.product.name}
                        </h4>
                        <p className="text-[11px] text-[#525A43] font-medium font-serif mt-0.5">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <div className="flex items-center border border-stone-200 rounded-lg bg-stone-50">
                          ...
                          <span className="px-2 text-xs font-bold text-[#2F3428]">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => onUpdateQuantity(item.product.id, item.quantity + 1)}
                  ...
                  <div className="bg-white p-3 rounded-xl border border-[#A1A696]/40 text-xs text-[#2F3428] flex items-center gap-2">
                    <Truck className="w-4 h-4 text-[#525A43] shrink-0" />
                    {subtotal >= 2000 ? (
